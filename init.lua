...
local DEFAULT_PROJECT_PATH = '~/dev/kaizogame/Editor'
...
-- -----------------------------------------------------------------------------
  {
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>-', function() oil.open() end)
    end
  },
-- -----------------------------------------------------------------------------
  {
...
-- -----------------------------------------------------------------------------
  {
    ...
    config = function()
      ...
      vim.lsp.config.clangd = {
        ...
        root_dir = function(bufnr) return root_for(bufnr, { 'compile_commands.json', '.git' }) end,
      ...
      local function ensure_started(server, bufnr)
        for _, c in ipairs(vim.lsp.get_clients({ bufnr = bufnr })) do
          if c.name == server then return end
        end
        local cfg = vim.lsp.config[server]
        if not cfg then return end
        local root = cfg.root_dir and cfg.root_dir(bufnr) or nil
        if not root then
          return
        end
      ...
      vim.api.nvim_create_autocmd('FileType', {
        pattern = vim.lsp.config.clangd.filetypes,
        callback = function(args) ensure_started('clangd', args.buf) end,
      })

      vim.api.nvim_create_autocmd('FileType', {
        pattern = vim.lsp.config.omnisharp.filetypes,
        callback = function(args) ensure_started('omnisharp', args.buf) end,
      })
...
vim.keymap.set('n', '<f5>', function()
  ...
  overseer.run_task({ name = 'build' }, function(task, success)
    ...
    task:subscribe('on_complete', function(task, status)
      ...
      if status == 'SUCCESS' then
        ...
        --   type = 'lldb',
        --   request = 'launch',
        --   name = 'LLDB: rebounder_editor',
        --   program = function() return vim.fn.getcwd() .. '/bin/rebounder_editor' end,
        --   cwd = '${workspaceFolder}',
        --   stopOnEntry = false,
        --   runInTerminal = true,
        --   args = {},
...
do
  ...
  vim.api.nvim_set_hl(0, 'TabLineSel', { link = 'DiffAdd' })
end
...
do
  local function setup_jump_to_project(num, path)
    vim.keymap.set('n', '<leader>'..num, function()
      vim.cmd('tcd '..path)
      vim.notify('tab jumped to '..path)
    end)
  end
  setup_jump_to_project(1, DEFAULT_PROJECT_PATH)
  setup_jump_to_project(2, '~/dev/kaizogame')
  setup_jump_to_project(3, '~/dev/webserver33/reboundergame.com')
  setup_jump_to_project(4, '~/dev/zine')
end
...
vim.keymap.set('n', '<leader>gl', '<cmd>Git log --all --graph --decorate --oneline --date=short --pretty=format:"%h %ad %an%d :: %s"<cr>')
vim.keymap.set('n', '<leader>gb', '<cmd>Git blame<cr>')

-- svn
vim.keymap.set('n', '<leader>ss', ':new<cr>:setlocal buftype=nofile bufhidden=hide noswapfile<cr>:set filetype=diff<cr>:%!svn st<cr>')
vim.keymap.set('n', '<leader>sd', ':new<cr>:setlocal buftype=nofile bufhidden=hide noswapfile<cr>:set filetype=diff<cr>:%!svn diff<cr>')
